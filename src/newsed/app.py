...
@click.option(
    ...
    help="Path to the user-defined script file.",
)
            ...
            try:
                ...
                for a_href in find_articles(soup, url, user_script):
                    if article_count >= a:
                        break
                    console.print(
                        f"[link={url + a_href.get('href')}]{make_bold(a_href.text)}[/link]"
                    )
                    article_count += 1
            except requests.RequestException as e:
                console.print(
                    f"Error fetching articles from {url}: {e}", style="bold red"
                )
        else:
            article_count = 0
